...
async function _SendTransaction(from, to, value, data, actionId) {
 ethereum.request({
  method: "eth_sendTransaction",
  params: [ {
   from: UTF8ToString(from),
   to: UTF8ToString(to),
   value: UTF8ToString(value),
   data: UTF8ToString(data)
  } ]
 }).then(transactionHash => {
  unityInstance.SendMessage("JSBridge", "JSEvent", JSON.stringify({
   name: "txHash",
   actionId: actionId,
   payload: transactionHash
  }));
  ethProvider.waitForTransaction(transactionHash).then(receipt => {
   unityInstance.SendMessage("JSBridge", "JSEvent", JSON.stringify({
    name: "result",
    actionId: actionId,
    payload: receipt
...
function _SignMessage(message, address, actionId) {
 const signature = ethereum.request({
  method: "personal_sign",
  params: [ UTF8ToString(message), UTF8ToString(address) ]
 }).then(signature => {
  unityInstance.SendMessage("JSBridge", "JSEvent", JSON.stringify({
   name: "result",
   actionId: actionId,
...
var dynCall_viiiifijiji = Module["dynCall_viiiifijiji"] = createExportWrapper("dynCall_viiiifijiji");
